...
data = pandas.read_table("results.txt", header=None, sep=" ")
sets = {}
archives = {}
...
for resolution in resolutions:
    ...
    ax.scatter(sets[resolution]["_0"], sets[resolution]["_1"], facecolor=(1.0, 1.0, 0.4), zorder=1, s=50)
...
for box in archives[resolution].boxes:
    ...
    # make a rectangle in the Y direction
    rect = matplotlib.patches.Rectangle((ll[0], ll[1] + resolution), 0.5 - ll[0], 0.5 - ll[1], lw=0, facecolor=(1.0,0.8,0.8), zorder=-10)
    ax.add_patch(rect)

    # make a rectangle in the X direction
    rect = matplotlib.patches.Rectangle((ll[0] + resolution, ll[1]), 0.5 - ll[0], 0.5 - ll[1], lw=0, facecolor=(1.0,0.8,0.8), zorder=-10)
    ax.add_patch(rect)
if resolution < 1e-3:
    spacing = 0.05
else:
...
ax.set_xticks(numpy.arange(0, 0.5, spacing))
ax.set_yticks(numpy.arange(0, 0.5, spacing))
...
ax.set_ylim(0, 0.5)
ax.set_title("Epsilon resolution: {0:.2g}".format(resolution))
ax.set_xlabel(r'$fp rate$')
ax.set_ylabel(r'$fn rate$')
